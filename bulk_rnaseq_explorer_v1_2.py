...
Version: bulk_rnaseq_explorer_v1_2
...
- Generate a QC readiness summary.
- Detect and attempt to parse a local mouse Ensembl-to-symbol gene map.
- Audit the legacy browser-only HTML as a migration map.
...
APP_VERSION = "bulk_rnaseq_explorer_v1_2"
...
def invalidate_downstream_results() -> None:
    ...
    st.session_state["qc_readiness"] = None
...
def _mapping_dataframe_from_object(data: Any) -> pd.DataFrame | None:
    ...
    if isinstance(data, list):
        rows = []
        ensembl_keys = ["ensembl_id", "EnsemblID", "ensembl", "ensembl_gene_id", "gene_id", "id"]
        symbol_keys = ["gene_symbol", "symbol", "Gene", "gene_name", "name", "external_gene_name"]
        for item in data:
...
def _extract_json_like_object_from_js(text: str) -> Any:
    ...
    if assignment_match:
        ...
        if first_brace == -1 or last_brace <= first_brace:
            raise ValueError("No JSON-like object found in JavaScript file.")
...
def audit_legacy_html_features() -> pd.DataFrame:
    """Lightweight audit of the legacy browser prototype as a migration map."""
    html_path = Path.cwd() / "index_v5.4.9.html"
    modules = [
        {
            "module": "Upload / parsing",
            "keywords": ["upload", "count matrix", "parse", "FileReader", "tsv", "csv"],
            "category": "Python rewrite + UI reference",
            "priority": "v1.x",
            "action": "Implemented in v1.2 as Streamlit upload and parser.",
            "notes": "Keep the old layout guidance, but validation and parsing belong in Python.",
        },
        {
            "module": "Sample grouping",
            "keywords": ["group", "assignment", "control", "treatment", "sample"],
            "category": "Direct UI migration + Python rewrite",
            "priority": "v1.x",
            "action": "Implemented in v1.2 with two editable group names and sample selectboxes.",
            "notes": "Browser modal patterns are simplified into Streamlit stateful controls.",
        },
        {
            "module": "Gene mapping",
            "keywords": ["ensembl", "symbol", "gene map", "mouse_ensembl_to_symbol"],
            "category": "Python rewrite using source_mapping",
            "priority": "v1.x",
            "action": "Detected and parsed when possible; not applied to analysis yet.",
            "notes": "Mapping should be centralized before DEG/pathway modules use identifiers.",
        },
        {
            "module": "QC summary",
            "keywords": ["QC", "qc-summary", "library size", "zero", "readiness"],
            "category": "Python rewrite",
            "priority": "v1.x",
            "action": "Implemented as QC readiness summary only.",
            "notes": "Plots remain deferred; readiness checks reduce bad downstream inputs.",
        },
        {
            "module": "Normalization",
            "keywords": ["normalize", "normalization", "logCPM", "vst", "rlog"],
            "category": "Python rewrite",
            "priority": "v1.x",
            "action": "Deferred.",
            "notes": "Use reproducible Python/R methods rather than browser-only calculations.",
        },
        {
            "module": "PCA",
            "keywords": ["PCA", "pca", "principal component"],
            "category": "Python sklearn rewrite",
            "priority": "v1.x",
            "action": "Deferred.",
            "notes": "Implement after normalization/QC data contracts are stable.",
        },
        {
            "module": "DEG analysis",
            "keywords": ["DEG", "differential", "log2FC", "pvalue", "padj", "DESeq2"],
            "category": "Replace with R/DESeq2 later",
            "priority": "v2.x",
            "action": "Deferred.",
            "notes": "Do not port hand-written browser statistics for inference.",
        },
        {
            "module": "Volcano plot",
            "keywords": ["volcano", "log2FC", "padj", "threshold"],
            "category": "Python/Plotly rewrite after DEG",
            "priority": "v2.x",
            "action": "Deferred.",
            "notes": "Depends on a trusted DEG result object.",
        },
        {
            "module": "Heatmap",
            "keywords": ["heatmap", "cluster", "z-score", "dendrogram"],
            "category": "Python/Plotly or matplotlib rewrite after DEG",
            "priority": "v2.x",
            "action": "Deferred.",
            "notes": "Should support selected genes and normalized expression matrix.",
        },
        {
            "module": "ORA",
            "keywords": ["ORA", "over-representation", "hypergeometric", "enrichment"],
            "category": "Replace with mature package later",
            "priority": "v3.x",
            "action": "Deferred.",
            "notes": "Needs stable gene universe and identifier mapping.",
        },
        {
            "module": "GSEA",
            "keywords": ["GSEA", "enrichment score", "NES", "permutation", "fgsea"],
            "category": "Replace with fgsea/gseapy later",
            "priority": "v3.x",
            "action": "Deferred.",
            "notes": "Use mature implementation after ranked DEG statistics are available.",
        },
        {
            "module": "Export",
            "keywords": ["export", "download", "save", "CSV", "TSV"],
            "category": "Python rewrite",
            "priority": "v2.x",
            "action": "Deferred.",
            "notes": "Exports should be generated from Streamlit/Python result objects.",
        },
        {
            "module": "UI controls",
            "keywords": ["button", "select", "input", "slider", "modal", "tab"],
            "category": "Direct UI reference",
            "priority": "v1.x",
            "action": "Partially implemented.",
            "notes": "Keep useful workflow affordances; avoid recreating complex custom JS.",
        },
        {
            "module": "State management",
            "keywords": ["state", "cache", "results", "current", "session"],
            "category": "Python Streamlit session_state rewrite",
            "priority": "v1.x",
            "action": "Implemented for input and validation state.",
            "notes": "Downstream invalidation is explicit to avoid stale results.",
        },
        {
            "module": "Known migration risks",
            "keywords": ["TODO", "warning", "error", "fallback", "manual", "prototype"],
            "category": "Defer",
            "priority": "Later",
            "action": "Tracked in audit notes.",
            "notes": "Major risks: browser math parity, identifier ambiguity, stale state, and reproducing inferential statistics.",
        },
    ]
    ...
    if not html_path.exists():
        return pd.DataFrame(
            [
                {
                    "Module": item["module"],
                    "Legacy feature detected": "Unclear",
                    "Evidence keywords/functions": "index_v5.4.9.html not found",
                    "Migration category": item["category"],
                    "Priority": item["priority"],
                    "v1.2 action": item["action"],
                    "Notes": item["notes"],
                }
                for item in modules
            ]
    ...
    labels = re.findall(r"<button[^>]*>(.*?)</button>", text, flags=re.IGNORECASE | re.DOTALL)
    searchable_text = "\n".join([text, " ".join(functions), " ".join(ids), " ".join(labels)]).lower()
    ...
    for item in modules:
        ...
        related_functions = [
            fn for fn in functions if any(keyword.lower().replace(" ", "") in fn.lower() for keyword in item["keywords"])
        ][:6]
        related_ids = [
            element_id for element_id in ids if any(keyword.lower().replace(" ", "-") in element_id.lower() for keyword in item["keywords"])
        ][:6]
        evidence_text_parts = evidence + [f"function:{fn}" for fn in related_functions] + [f"id:{element_id}" for element_id in related_ids]
        detected = "Yes" if evidence_text_parts else "No"
        rows.append(
            {
                "Module": item["module"],
                "Legacy feature detected": detected,
                "Evidence keywords/functions": ", ".join(evidence_text_parts[:12]) if evidence_text_parts else "No direct keyword match",
                "Migration category": item["category"],
                "Priority": item["priority"],
                "v1.2 action": item["action"],
                "Notes": item["notes"],
...
def validate_counts(counts_df: pd.DataFrame | None, gene_id_column: str | None) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "library_size_imbalance_ratio": None,
    ...
    if not count_values.empty:
        library_sizes = count_values.fillna(0).sum(axis=0)
        zero_library_samples = [str(sample) for sample, value in library_sizes.items() if value == 0]
        nonzero_library_sizes = library_sizes[library_sizes > 0]
        imbalance_ratio = None
        if not nonzero_library_sizes.empty:
            min_nonzero = float(nonzero_library_sizes.min())
            max_library = float(library_sizes.max())
            imbalance_ratio = max_library / min_nonzero if min_nonzero > 0 else None
        summary["zero_library_samples"] = zero_library_samples
        summary["library_size_imbalance_ratio"] = round(imbalance_ratio, 4) if imbalance_ratio is not None else None
        summary["sample_library_sizes"] = {str(sample): float(value) for sample, value in library_sizes.items()}
    else:
        zero_library_samples = []
        imbalance_ratio = None
    ...
    if zero_library_samples:
        checks.append(
            _make_check(
                "Counts",
                "Zero-library samples",
                "Error",
                "One or more samples have total count = 0.",
                ", ".join(zero_library_samples[:10]),
            )
        )
    else:
        checks.append(_make_check("Counts", "Zero-library samples", "Pass", "No zero-library samples detected.", 0))

    if imbalance_ratio is not None and imbalance_ratio > 5:
        checks.append(
            _make_check(
                "Counts",
                "Extremely high library size imbalance",
                "Warning",
                "Max library size / min nonzero library size is greater than 5.",
        ...
        checks.append(
            _make_check(
                ...
                "Extremely high library size imbalance",
                "Pass",
                "No extreme library size imbalance detected.",
                round(imbalance_ratio, 4) if imbalance_ratio is not None else "NA",
            )
        )
...
def validate_group_assignments(
    ...
    if sample_group_assignments:
        checks.append(_make_check("Groups", "Group assignment exists", "Pass", "Group assignment dictionary exists.", len(sample_group_assignments)))
    else:
        checks.append(_make_check("Groups", "Group assignment exists", "Error", "No sample group assignments have been made.", 0))
    ...
    if group_1_name == group_2_name:
        checks.append(_make_check("Groups", "Group names are not identical", "Error", "Group 1 and Group 2 names must be different."))
    else:
        checks.append(_make_check("Groups", "Group names are not identical", "Pass", "Group names are distinct."))
    ...
    if len(selected_groups) != 2:
        checks.append(
            _make_check(
                "Groups",
                "At least two groups have assigned samples",
                "Error",
        ...
        checks.append(_make_check("Groups", "At least two groups have assigned samples", "Pass", "Both groups have assigned samples.", 2))
...
def compute_qc_readiness(report: dict[str, Any]) -> dict[str, str]:
    """Summarize whether validated inputs can proceed to future QC and DEG modules."""
    checks = report.get("checks", [])
    status = report.get("status", "Not run")
    count_errors = [
        check
        for check in checks
        if check.get("Section") == "Counts" and check.get("Level") == "Error"
    ]
    ...
    if status == "Not run":
        return {
            "Ready for QC": "No",
            "Ready for DEG": "No",
            "Reason": "Validation has not been run.",
        }
    ...
    if count_errors:
        return {
            "Ready for QC": "No",
            "Ready for DEG": "No",
            "Reason": "Count matrix has serious errors that must be fixed before QC.",
        }
    ...
    if status == "Failed":
        return {
            "Ready for QC": "Yes",
            "Ready for DEG": "No",
            "Reason": "Counts are usable for QC, but group assignment errors block DEG.",
        }

    if status == "Warning":
        return {
            "Ready for QC": "Yes",
            "Ready for DEG": "Warning",
            "Reason": "Inputs can proceed to QC, but warnings should be reviewed before DEG.",
        }

    return {
        "Ready for QC": "Yes",
        "Ready for DEG": "Yes",
        "Reason": "No validation errors or warnings detected.",
    }
...
def run_full_validation() -> dict[str, Any]:
    ...
    if counts_df is None:
        ...
        st.session_state["qc_readiness"] = compute_qc_readiness(report)
        return report
...
def _display_gene_map_status() -> None:
    ...
    st.write(f"Gene map source path: `{status.get('path') or status.get('source_path') or 'None'}`")
    ...
    if status.get("parsed") and st.session_state.get("gene_map_df") is not None:
        st.dataframe(st.session_state["gene_map_df"].head(5), use_container_width=True, hide_index=True)


def reset_group_assignments_for_samples(sample_columns: list[str]) -> None:
    """Reset assignment records when count samples change."""
...
def update_group_names_safely(old_group_1: str, new_group_1: str, old_group_2: str, new_group_2: str) -> None:
    """Rename existing assignments when group labels change."""
    updated = {}
...
def render_sidebar() -> None:
    ...
    unassigned_count = sum(
        1 for sample in sample_columns if assignments.get(sample, "Unassigned") not in {group_1, group_2}
    )
    n_genes = int(st.session_state["counts_df"].shape[0]) if st.session_state["counts_df"] is not None else 0
    ...
    st.sidebar.write(f"Number of genes: {n_genes}")
    st.sidebar.write(f"Number of samples: {len(sample_columns)}")
    st.sidebar.write(f"Groups assigned: {'Yes' if groups_assigned else 'No'}")
    st.sidebar.write(f"Group 1: {group_1}, {group_1_count} samples")
    st.sidebar.write(f"Group 2: {group_2}, {group_2_count} samples")
    st.sidebar.write(f"Unassigned samples: {unassigned_count}")
    st.sidebar.write(f"Gene map detected: {'Yes' if st.session_state['gene_map_status'].get('detected') else 'No'}")
    st.sidebar.write(f"Gene map parsed: {'Yes' if st.session_state['gene_map_status'].get('parsed') else 'No'}")
...
def _reset_counts_dependent_state(counts_df: pd.DataFrame, file_name: str) -> None:
    ...
    reset_group_assignments_for_samples(st.session_state["sample_columns"])
...
def render_upload_count_matrix_tab() -> None:
    ...
    example_layout = pd.DataFrame(
        {
            "EnsemblID / Gene symbols": ["ENSMUSG000000029380", "CXCL1"],
            "Sample 1": [3, 3],
            "Sample 2": [784, 784],
            "Sample 3": [53, 53],
        }
    ...
    if counts_df is not None:
        ...
        selected_gene_id = st.selectbox(
            ...
            key="gene_id_column_selector_v1_2",
        )
...
def render_validation_tab() -> None:
    ...
    st.markdown("#### QC readiness summary")
    qc_readiness = st.session_state.get("qc_readiness") or compute_qc_readiness(report)
    readiness_df = pd.DataFrame([qc_readiness])
    st.dataframe(readiness_df, use_container_width=True, hide_index=True)


def render_legacy_migration_map_tab() -> None:
    """Render legacy HTML audit and migration priority map."""
    st.subheader("Legacy Migration Map")
    html_path = Path.cwd() / "index_v5.4.9.html"
    if not html_path.exists():
        st.warning("index_v5.4.9.html was not found. Legacy audit is shown as unclear.")

    if st.session_state.get("legacy_migration_map") is None:
        st.session_state["legacy_migration_map"] = audit_legacy_html_features()

    with st.expander("Legacy HTML migration audit", expanded=True):
        st.write(
            "This lightweight audit treats the browser-only HTML as a product reference. "
            "It scans keywords, function names, IDs, and button labels to guide Python migration priorities."
        )
        st.dataframe(st.session_state["legacy_migration_map"], use_container_width=True, hide_index=True)
...
def main() -> None:
    ...
    upload_tab, groups_tab, validation_tab, legacy_tab = st.tabs(
        ["Upload Count Matrix", "Assign Sample Groups", "Input Validation", "Legacy Migration Map"]
    )
    with upload_tab:
    ...
    with legacy_tab:
        render_legacy_migration_map_tab()
