...
Version: bulk_rnaseq_explorer_v1_3
...
- Show concise internal readiness warnings without exposing a full validation/debug tab.
...
APP_VERSION = "bulk_rnaseq_explorer_v1_3"
...
def detect_local_resources() -> pd.DataFrame:
    """Detect local resources silently for internal loading."""
    cwd = Path.cwd()
    resource_specs = [
...
def render_sidebar() -> None:
    """Render compact product workflow and project status."""
    st.sidebar.title("Workflow")
    st.sidebar.success("1. Upload Count Matrix")
    st.sidebar.success("2. Assign Sample Groups")
    st.sidebar.caption("3. QC Overview - Coming soon / Locked")
    ...
    group_2_count = sum(1 for sample in sample_columns if assignments.get(sample) == group_2)
    unassigned_count = sum(1 for sample in sample_columns if assignments.get(sample, "Unassigned") not in {group_1, group_2})
    groups_assigned = group_1_count >= 1 and group_2_count >= 1
    raw_genes = int(st.session_state["raw_counts_df"].shape[0]) if st.session_state["raw_counts_df"] is not None else 0
    processed_genes = int(st.session_state["processed_counts_df"].shape[0]) if st.session_state["processed_counts_df"] is not None else 0
    ...
    mode_label = {
        ...
        "mixed": "Mixed",
        "unknown": "Unknown",
    }.get(st.session_state["gene_id_mode"], "Unknown")

    st.sidebar.divider()
    st.sidebar.subheader("Project status")
    st.sidebar.write(f"Count matrix uploaded: {'Yes' if st.session_state['raw_counts_df'] is not None else 'No'}")
    st.sidebar.write(f"Number of raw genes: {raw_genes}")
    st.sidebar.write(f"Number of processed genes: {processed_genes}")
    ...
    st.sidebar.write(f"Gene ID mode: {mode_label}")
    st.sidebar.write(f"Gene symbols converted: {summary.get('converted_to_gene_symbols', 0)}")
    st.sidebar.write(f"Duplicated gene symbols merged: {summary.get('duplicated_gene_symbols', 0)}")
...
def render_gene_symbol_conversion_section() -> None:
    """Render concise gene ID conversion status."""
    ...
    col1.metric("Mapping source", summary.get("mapping_source", "Not available"))
    col2.metric("Total genes", f"{summary.get('total_genes', 0):,}")
    col3.metric("Converted to gene symbols", f"{summary.get('converted_to_gene_symbols', 0):,}")
    col4.metric("Unconverted genes", f"{summary.get('unconverted_genes', 0):,}")
...
def render_duplicate_gene_section() -> None:
    ...
    col1.metric("Duplicated gene symbols", f"{duplicated_count:,}")
    col2.metric("Processed genes after merge", f"{processed_genes:,}")

    duplicate_summary_df = st.session_state["duplicate_summary_df"]
    if duplicated_count > 0:
        with st.expander("View duplicated genes"):
            st.dataframe(duplicate_summary_df, use_container_width=True, hide_index=True)
...
def render_upload_count_matrix_tab() -> None:
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["tsv", "txt", "csv"],
        key="count_matrix_uploader_v1_3",
    ...
    selected_gene_column = st.selectbox(
        ...
        key="gene_id_column_selector_v1_3",
    )
    ...
    st.write(f"Detected samples: `{_format_sample_list(sample_columns)}`")
...
def main() -> None:
    ...
    upload_tab, groups_tab = st.tabs(["Upload Count Matrix", "Assign Sample Groups"])
    with upload_tab:
